...
uses
  ...
  FireDAC.Stan.Error, JsonDataObjects;

procedure TUsersController.ChangeCurrentUserPassword;
var
...
begin
  ...
  try
    lUser := TMVCActiveRecord.GetOneByWhere<TUserWithPassword>('email = ? and not deleted', [Context.LoggedUser.UserName], True);
    try
...
var
  lUser: TUser;
begin
  if Context.LoggedUser.CustomData['user_id'].ToInteger = ID then
  begin
